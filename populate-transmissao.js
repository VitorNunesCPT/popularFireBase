...
const { transmissao5w2h } = require("./data/transmissao-data");
...
async function populateTransmissao() {
  try {
    ...
    const transmissao5w2hRef = card2Ref
      .collection("detalhes")
      .doc("transmissao");

    const timestamp = new Date();

    batch.set(transmissao5w2hRef, {
      title: "Transmissão da Tuberculose: Avaliação 5W2H",
      description:
        "Conteúdo estruturado sobre a transmissão da TB organizado em O Quê, Por Quê, Onde, Quando, Quem, Como e Quanto.",
      order: 1,
      data: transmissao5w2h,
      createdAt: timestamp,
      updatedAt: timestamp,
    });
    ...
    console.log(
      '✅ Documento "transmissao-5w2h" criado com sucesso em infoCards/card_2/detalhes.'
    );
...
async function main() {
  ...
  switch (command) {
    ...
    default:
      console.log(
        "📖 Comandos disponíveis para subcoleção detalhes do card_2:"
      );
