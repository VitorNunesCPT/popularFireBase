...
async function populateInteracoesMedicamentosas() {
  try {
    ...
    const docRef = card6Ref
      .collection("detalhes")
      .doc("interacoes-medicamentosas-5w2h");
    const timestamp = new Date();

    batch.set(docRef, {
      title: "Interações Medicamentosas da Tuberculose: Avaliação 5W2H",
      description:
        "Conteúdo estruturado sobre interações medicamentosas organizado em O Quê, Por Quê, Onde, Quando, Quem, Como e Quanto.",
      order: 1,
      data: interacoesMedicamentosas5w2h,
      createdAt: timestamp,
      updatedAt: timestamp,
    });
    ...
    console.log(
      '✅ Documento "interacoes-medicamentosas-5w2h" criado com sucesso em infoCards/card_6/detalhes.'
    );
...
async function main() {
  ...
  switch (command) {
    ...
    default:
      console.log(
        "📖 Comandos disponíveis para subcoleção detalhes do card_6:"
      );
      ...
      console.log("");
      console.log(
        "💡 Exemplo: node populate-interacoes-medicamentosas.js populate"
      );
